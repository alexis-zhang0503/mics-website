...
项目没有后端、数据库、前端框架或 npm 依赖。访问根首页时默认进入英文版 `en/index.html`；需要中文首页时可通过语言切换进入 `index.html?lang=cn`。
...
中文页面位于项目根目录，英文页面位于 `en/` 目录。根首页默认跳转到英文版；英文首页的 `CN` 链接指向 `index.html?lang=cn`，用于绕过默认跳转并进入中文首页。新增或修改真实内容时，需要同步更新对应语言页面，并保持顶部 `EN / CN` 语言切换链接指向对应页面。
